    ...
    # VPT
        ...
        if args["model_name"] == "sema":
            ...
            tuning_config = EasyDict(
                ...
                # Hybrid SEMA Configs
                router_attn_dim=args.get("router_attn_dim", 128),
                exp_k_std=args.get("exp_k_std", 3.0)
            )
...
class BaseNet(nn.Module):
    ...
    def extract_vector(self, x):
        if self.model_type == 'cnn':
            ...
            # [FIX] Ensure this returns the tensor for all ViT models by default
            return self.backbone(x)["features"]
...
# ... (Keep IncrementalNet, CosineIncrementalNet, DERNet, SimpleCosineIncrementalNet, SimpleVitNet unchanged) ...
...
class SEMAVitNet(BaseNet):
    ...
    def extract_vector(self, x):
        # [FIX] Return the tensor, not the dict, for NCM
        return self.backbone(x)["features"]
    ...
    def forward(self, x):
        ...
        # [FIX] Standard Linear FC forward (no dictionary logic needed)
        logits = self.fc(x)
            
        out.update({"logits": logits})
        return out

# ... (Keep PromptVitNet, CodaPromptVitNet, etc. unchanged) ...
# l2p and dualprompt
